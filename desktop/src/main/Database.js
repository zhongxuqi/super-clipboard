...
export default {
  insert: function (clipboardMsg, callback) {
    db.run('insert into ' + TABLE_NAME + ' (type,content,extra,create_time,update_time) values (?,?,?,?,?)', [clipboardMsg.type, clipboardMsg.content, clipboardMsg.extra, clipboardMsg.create_time, clipboardMsg.update_time], function (res, err) {
      db.all('SELECT id,type,content,extra,create_time,update_time FROM ' + TABLE_NAME + ' order by create_time DESC limit 1', function (err, rows) {
  ...
  listAll: function (callback) {
    db.all('SELECT id,type,content,extra,create_time,update_time FROM ' + TABLE_NAME + ' order by create_time ASC', function (err, rows) {
      if (err !== undefined && err != null) {
        console.log(err)
        return
      }
